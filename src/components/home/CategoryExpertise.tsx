    ...
    return (
        <section className="w-full bg-transparent">
            <div className="container mx-auto px-2 py-6">
                <div className="bg-[#dcd7c8]/40 rounded-xl border border-[#c1bcae]/40 p-5 overflow-hidden">
                    {/* Header */}
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-3">
                            <div className="w-1 h-6 bg-[var(--color-primary)] rounded-full"></div>
                            <h3 className="text-lg font-bold text-zinc-900">Top Categories</h3>
                        </div>
                    ...
                    <div className="category-carousel-wrapper">
                        <div className="category-carousel-track">
                            {displayCategories.length > 0 ? displayCategories.map((cat, idx) => (
                                ...
                                >
                                    <div className="w-[80px] h-[80px] rounded-2xl bg-gradient-to-br from-[#dcd7c8]/60 to-[#dcd7c8]/80 border border-[#c1bcae]/40 flex items-center justify-center overflow-hidden transition-all duration-300 group-hover:border-[var(--color-primary)]/30 group-hover:shadow-xl group-hover:scale-110 group-hover:from-[var(--color-primary)]/5 group-hover:to-[var(--color-primary)]/10">
                                        {cat.image ? (
                                    ...
                                    <span className="text-[13px] text-zinc-800 text-center font-medium group-hover:text-[var(--color-primary)] transition-colors whitespace-nowrap">{cat.name}</span>
                                </Link>
                            )) : (
                                [...Array(8)].map((_, i) => (
