...
const ProductFormInner = () => {
    ...
    const [formData, setFormData] = useState<any>({
        ...
        // Jamdani / Additional Tabs
        deliveryInfo: '', careInfo: '', paymentInfo: '', termsInfo: '',
    });
    ...
    useEffect(() => {
        if (isEditing && productToEdit?.data) {
            ...
            setFormData({
                ...
                paymentInfo: prod.paymentInfo || '',
                termsInfo: prod.termsInfo || '',
            });
        }
        ...
        <div className="max-w-7xl mx-auto space-y-6 pb-32">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    {/* ── 6. Additional Info (Tabs) ────────────────── */}
                    <div className="bg-white p-6 rounded-md border border-gray-200 shadow-sm space-y-6">
                        <SectionHeader icon={<FiFileText size={20} />} title="Detailed Information (Tabs)" color="bg-emerald-50 text-emerald-600" />
                        <p className="text-xs text-gray-500 -mt-2">এই ইনফরমেশনগুলো প্রোডাক্ট পেজের Tab এ সুন্দরভাবে দেখাবে। জামদানি শাড়ির ক্ষেত্রে Care (ধোয়ার নিয়ম) ও Delivery ডিটেইলস দেওয়া জরুরি।</p>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><FiTruck className="text-blue-500" /> Delivery Details</label>
                                <textarea name="deliveryInfo" rows={3} placeholder="e.g. Inside Dhaka: 80 TK (3-5 days), Outside Dhaka: 150 TK (5-7 days)..." className="w-full px-4 py-3 bg-white border border-gray-200 rounded-md outline-none focus:border-blue-400 transition-all text-sm" value={formData.deliveryInfo} onChange={handleChange}></textarea>
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><FiDroplet className="text-pink-500" /> Care Instructions</label>
                                <textarea name="careInfo" rows={3} placeholder="e.g. Hand wash only with cold water. Avoid direct sunlight..." className="w-full px-4 py-3 bg-white border border-gray-200 rounded-md outline-none focus:border-pink-400 transition-all text-sm" value={formData.careInfo} onChange={handleChange}></textarea>
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><FiDollarSign className="text-green-500" /> Payment Information</label>
                                <textarea name="paymentInfo" rows={3} placeholder="e.g. Bkash, Nagad, Rocket or Cash on Delivery available..." className="w-full px-4 py-3 bg-white border border-gray-200 rounded-md outline-none focus:border-green-400 transition-all text-sm" value={formData.paymentInfo} onChange={handleChange}></textarea>
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><FiShield className="text-orange-500" /> Return & Refund Terms</label>
                                <textarea name="termsInfo" rows={3} placeholder="e.g. 7 days return policy if the product is damaged..." className="w-full px-4 py-3 bg-white border border-gray-200 rounded-md outline-none focus:border-orange-400 transition-all text-sm" value={formData.termsInfo} onChange={handleChange}></textarea>
                            </div>
                        </div>
                    </div>
